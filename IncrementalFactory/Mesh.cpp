...
void Mesh::loadMtl(const std::string& path, std::map<std::string, Material>& materials) {
    if (!fileExist(path)) {
    ...
    while (std::getline(file, line)) {
        ...
        if (key == "newmtl") {
            ...
            materials[name].diffuseMap = loadTexture(dir + fn);
        }
        else if (key == "bump" || key == "map_Bump" || key == "norm") {
            std::string fn; iss >> fn;
            std::string dir = path.substr(0, path.find_last_of("/\\") + 1);
            materials[name].normalMap = loadTexture(dir + fn);
        }
    }
    file.close();
}
...
std::vector<Mesh> Mesh::loadMeshes(const std::string& path) {
    ...
    Mesh::loadMtl(mtl_path, materials);
